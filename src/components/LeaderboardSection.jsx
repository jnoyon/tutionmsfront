    ...
    <div className="border border-gray-300 rounded-md py-3 mb-3 bg-white">
      ...
      <div className="flex gap-3 justify-center items-end">
        {sorted.map((student, index) => {
          ...
          const imgSize = index === 0 ? 'w-24 h-24 md:w-64 md:h-64 -mb-12 md:-mb-28' : 'w-20 h-20 md:w-40 md:h-40 -mb-10 md:-mb-20';
          ...
          return (
            <div key={student.id} className="flex flex-col items-center">
              <div className={`${bgColor} flex items-end justify-center rounded-t-lg relative`}>
                ...
                />
                <span className="absolute -top-2 text-white px-2 py-1 rounded-full text-sm md:text-xl md:-top-4 bg-black/50">
                  {position}
                </span>
              </div>
              <div className={`mt-10 text-center ${index === 0 ? 'md:mt-28' : 'md:mt-20'}`}>
