          ...
          if (!info[data.quizId]) {
            ...
            const quizSnap = await getDocs(
              query(
                ...
                where("__name__", "==", data.quizId)
              )
            );
            const title = !quizSnap.empty
              ? quizSnap.docs[0].data().title
              : data.quizId;
    ...
    <div className="bg-gradient-to-l from-yellow-50 via-red-50 to-blue-50 py-5">
      ...
      <div className="border border-gray-300 rounded-lg p-3 bg-white mx-auto w-11/12">
        <div className="text-center border-b border-gray-300 py-2 mb-2">
          ...
          />
          <b>{student.name}</b>
          <p> ব্যাচ-{student.batch} </p>
        </div>
        <div className="flex flex-col gap-1 text-sm md:text-base">
          <p>
            <strong>স্টুডেন্ট আইডি:</strong> {student.studentId}
          </p>
          <p>
            <strong>মোবাইল:</strong> {student.phone}
          </p>
          <p>
            <strong>পিতার নাম:</strong> {student.fatherName}
          </p>
          <p>
            <strong>ঠিকানা:</strong> {student.address}
          </p>
          <p>
            <strong>শিক্ষা প্রতিষ্ঠান:</strong> {student.instituteName}
          </p>
          <p>
            <strong>ভর্তি অবস্থা:</strong>{" "}
            {student.isActive ? "একটিভ" : "ইন-একটিভ"}
          </p>
          <Link className="btn btn-sm my-2 btn-primary" to="/profile">
            {" "}
            তথ্য পরিবর্তন করুন{" "}
          </Link>
        </div>
          ...
          <table className="table-auto border-collapse border border-gray-400 w-full text-sm md:text-base">
            <thead>
              ...
              <tr>
                ...
                <th className="border border-gray-300 p-1.5">
                  প্রাপ্ত নাম্বার
                </th>
                <th className="border border-gray-300 p-1.5">
                  সর্বোচ্চ নাম্বার
